...
/*   Updated: 2024/01/16 16:13:40 by ataboada         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_eat(t_philo *p)
{
	if (p->id % 2 == 0)
	{
		pthread_mutex_lock(&p->data->fork_mtx[p->fork_l]);
		pthread_mutex_lock(&p->data->fork_mtx[p->fork_r]);
	}
	else
	{
		pthread_mutex_lock(&p->data->fork_mtx[p->fork_r]);
	...
	usleep(p->data->time_to_eat * 1000);
	pthread_mutex_unlock(&p->data->fork_mtx[p->fork_l]);
	pthread_mutex_unlock(&p->data->fork_mtx[p->fork_r]);
	pthread_mutex_lock(&p->data->eat_mtx);
	p->times_eaten++;
